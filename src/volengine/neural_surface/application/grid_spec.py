"""The mesh a learned surface is published on. Configuration, not mathematics.

**A deliberate twin of ``parametric_pricing/application/grid_spec.py``** (rule 6: no context
imports another). The duplication is load-bearing here rather than merely tolerated: the two
producers publish onto the *same* mesh so that Design 6.5's distance between their surfaces is a
node-by-node subtraction rather than an interpolation of one onto the other's axis. Two
configurations that happen to agree is what makes that comparison honest -- one shared object
would make it true by construction and stop measuring anything about the wiring.
...
**Only the moneyness axis is configured.** The tenor axis is whatever the market quoted: one node
per expiry in the snapshot, with the expiry instant and forward that came with it. And here that
is a *choice* rather than a constraint -- unlike SVI, a network can be evaluated at any tenor at
all, since it sees the surface as one function of ``(k, T)`` with parameters shared across
expiries. Publishing on the quoted tenors anyway is what keeps the two producers comparable node
for node; evaluating this one on a denser axis would be publishing rows no market data stands
behind, and the comparison would then be measuring an interpolation choice.
"""
