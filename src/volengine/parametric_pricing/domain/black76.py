"""Scalar Black-76: the one place in this context that turns a volatility into a price.

Black-76 is Black-Scholes rewritten on the **forward** instead of the spot. The underlying of
the formula is a forward contract, which costs nothing to enter, so the drift term disappears
and no interest rate survives anywhere in it except the single ``discount`` factor that carries
the premium back to today::

    C = D * (F * N(d1) - K * N(d2))
    P = D * (K * N(-d2) - F * N(-d1))
    d1 = (log(F / K) + w / 2) / sqrt(w),  d2 = d1 - sqrt(w),  w = vol^2 * T

That shape is the reason this module knows nothing about markets. The forward is built upstream
by Market Data, which is where the venue's expiry convention, its underlying reference and its
carry live (ADR-002); by the time a number reaches this function it is a homogeneous forward,
tenor and strike, and there is no way left to ask which exchange it came from. A crypto
perpetual, an equity index future and a synthetic parity forward all price through the same
three lines, and the calibrator that sits on top of them never learns the difference.

**Own implied volatilities, not the venue's.** The calibration is run against volatilities this
module inverts from the mid (Design 4.5). An exchange's published IV is the output of the
exchange's own model, its own forward and its own expiry convention; fitting SVI to it would be
fitting our model to someone else's model rather than to the market, and every convention
mismatch would enter the surface disguised as a smile feature. The venue's IV is ingested as a
comparison column, never as an input.

**Pure ``math``, deliberately.** No scipy, no numpy, and rule 3 would forbid jax and torch here
anyway. Three reasons, in increasing order of importance:

1. Cost. ``scipy.stats.norm.cdf`` measures at roughly 19 us per scalar call in this environment
   against 0.035 us for the ``math.erfc`` form below -- about 550x -- because every call walks
   the generic ``rv_continuous`` machinery. The inversion loop runs per quote per snapshot, so
   that factor lands on the hot path of the whole engine.
2. Testability. The rules in ``*/domain/`` have to be checkable without a GPU, a JIT warm-up or
   an optimiser, which is the point of the import rule that keeps them here.
3. Reference status. The JAX Black-76 written in F3 will be tested *against this module*: the
   vectorised implementation is fast, and this one is the definition of what fast is supposed to
   agree with. A reference has to be simple and must not drift when a dependency changes its
   numerics in a patch release.
"""
...
class OptionKindP(StrEnum):
    ...
    A third spelling of the same idea, next to Market Data's ``OptionKindD`` and the published
    ``OptionKind`` in ``contracts/``, and the duplication is the architecture working rather than
    debt: no context imports another context (rule 6), and the domain does not import the
    contracts at all (rule 3). Unifying them into one canonical enum would put a shared type on
    the boundary and make every context's vocabulary hostage to every other's -- the exact
    anti-pattern the bounded contexts exist to prevent. The ``P`` suffix keeps the two spellings
    legible side by side inside ``application/acl.py``, which is the only module allowed to see
    both.
...
def _intrinsic(forward: float, strike: float, kind: OptionKindP, discount: float) -> float:
    """Discounted intrinsic value: the ``vol -> 0`` limit, and the lower no-arbitrage bound."""
    if kind is OptionKindP.CALL:
        return discount * max(forward - strike, 0.0)
    return discount * max(strike - forward, 0.0)


def _ceiling(forward: float, strike: float, kind: OptionKindP, discount: float) -> float:
